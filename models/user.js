...
class User {
  ...
  static async register({ username, password, first_name, last_name, phone }) {
    let hashedPsw = await bcrypt.hash(password, BCRYPT_WORK_FACTOR);
    const result = await db.query(`
      INSERT INTO users (
        username, password, first_name, last_name, phone, join_at, last_login_at)
        VALUES ($1, $2, $3, $4, $5, current_timestamp, current_timestamp)
        RETURNING username, password, first_name, last_name, phone`,
      [username, hashedPsw, first_name, last_name, phone]
    );
  ...
  static async messagesFrom(username) {
    let result = await db.query(`
      SELECT m.id, u.first_name, u.last_name, u.phone, m.body, m.sent_at, m.read_at
      FROM messages AS m
  ...
  static async messagesTo(username) {
    let result = await db.query(`
      ...
      FROM messages AS m
      JOIN users AS u ON m.from_username = username
